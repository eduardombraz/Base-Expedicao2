...
raw = f.read(1000000)
enc_detected = chardet.detect(raw).get("encoding") or "utf-8"
except Exception:
enc_detected = "utf-8"

encodings = [enc_detected, "utf-8", "latin-1", "cp1252"]  
seps = [",", ";", "\t"]  
...
for encoding in encodings:  
    for sep in seps:  
        try:  
            ...
            # Se só veio 1 coluna, provavelmente separador errado — tenta próximo  
            if df.shape[1] == 1:  
                continue  
            return df  
        except Exception as e:  
...
# Tentativa final sem sep explícito com o melhor encoding detectado  
try:  
